...
contract('Claim: Assessment', function([
           owner, member1, member2, member3, member4, member5, staker1, staker2,
           coverHolder, notMember
         ]) {
  ...
  before(async function() {
    ...
    await mcr.addMCRData(await getValue(toWei(2), pd, mcr), toWei(100),
                         toWei(2), [ '0x455448', '0x444149' ], [ 100, 65407 ],
                         20181011);
    ...
    describe('Voting is not closed yet', function() {
      describe('CA not voted yet', function() {
        describe('All CAs rejects claim', function() {
          ...
          it('8.2 should let claim assessors to vote for claim assessment',
             async function() {
               ...
               (finalCAVoteTokens[1] - initialCAVoteTokens[1])
                   .should.be.equal(tokens * 3);
          ...
          it('8.3 should not let claim assessors to vote for 2nd time in same claim id',
             async function() {
               await assertRevert(
                   cl.submitCAVote(claimId, -1, {from : member2}));
             });
          ...
          it('8.6 should not able to vote after voting close',
             async function() {
               await assertRevert(
                   cl.submitCAVote(claimId, 1, {from : member1}));
             });
        ...
        describe('All CAs accept claim', function() {
          ...
          before(async function() {
            ...
            initialStakedTokens1 =
                await tf.getStakerLockedTokensOnSmartContract(staker1,
                                                              smartConAdd, 0);
            initialStakedTokens2 =
                await tf.getStakerLockedTokensOnSmartContract(staker2,
                                                              smartConAdd, 1);
          });

          it('8.9 should let claim assessor to vote for claim assessment',
             async function() {
               await cl.submitCAVote(claimId, 1, {from : member1});
               await cl.submitCAVote(claimId, 1, {from : member2});
               await cl.submitCAVote(claimId, 1, {from : member3});
          ...
          it('8.10 should not able to vote after voting closed',
             async function() {
               const now = await latestTime();
          ...
          it('8.11 orcalise call should be able to change claim status',
             async function() {
               let apiid =
                   await pd.allAPIcall((await pd.getApilCallLength()) - 1);
      ...
      describe('CA not voted', function() {
        ...
        it('8.14 oracalise call should open voting for members after CA voting time expires',
           async function() {
             let apiid =
                 await pd.allAPIcall((await pd.getApilCallLength()) - 1);
             await P1.__callback(apiid, '');
        ...
        describe('Member not voted', function() {
          ...
          describe('After Member vote closing time', function() {
            ...
            it('8.16 oracalise call should change claim status ',
               async function() {
                 let apiid =
                     await pd.allAPIcall((await pd.getApilCallLength()) - 1);
                 await P1.__callback(apiid, '');
        ...
        describe('Member rejects claim', function() {
          ...
          it('8.18 member should be able to cast vote', async function() {
            ...
            let claimed = await cr.getRewardAndClaimedStatus(0, claimId,
                                                             {from : member1});
            let claimed1 =
                await cr.getRewardAndClaimedStatus(0, 0, {from : member1});
          ...
          it('8.19 should not be able to cast vote by CA in member voting',
             async function() {
               await assertRevert(
                   cl.submitCAVote(claimId, 1, {from : member1}));
             });

          it('8.20 should not let member to vote for 2nd time in same claim id',
             async function() {
               await assertRevert(
                   cl.submitMemberVote(claimId, -1, {from : member1}));
             });
          it('8.21 member should not be able to transfer any tokens',
             async function() {
          ...
          it('8.22 should not able to vote after voting closed',
             async function() {
               const now = await latestTime();
               closingTime =
                   new BN(maxVotingTime.toString()).add(new BN(now.toString()));
  ...
  describe('Member not locked tokens for Claim Assessment', function() {
    before(async function() {
      ...
      var vrsdata = await getQuoteValues(coverDetails, toHex('ETH'),
                                         coverPeriod, smartConAdd, qt.address);
      await P1.makeCoverBegin(smartConAdd, toHex('ETH'), coverDetails,
                              coverPeriod, vrsdata[0], vrsdata[1], vrsdata[2],
                              {from : coverHolder, value : coverDetails[1]});
      coverID = await qd.getAllCoversOfUser(coverHolder);
  ...
  describe('Expire Cover', function() {
    it('8.27 CSA should not change while ExpireCover if cover status is 1',
       async function() {
         const now = await latestTime();
  ...
  describe('CA Pool have insuffecient funds for payout', function() {
    before(async function() {
      ...
      await tc.lock(CLA, ether(400), duration.days(300), {from : member4});
    ...
    it('8.29 Retry Payout 60 times and should not take action from 61st attempt',
       async function() {
         await tf.upgradeCapitalPool(DAI.address);
         let apiid;
         let clid = (await cd.actualClaimLength()) - 1;
         ...
         coverID = await qd.getAllCoversOfUser(coverHolder);
         let coveStatus =
             await qd.getCoverStatusNo(coverID[coverID.length - 1]);
    ...
    it('8.30 Payout fails for 1st time and later complete', async function() {
      ...
      var vrsdata = await getQuoteValues(coverDetailsDai, toHex('DAI'),
                                         coverPeriod, smartConAdd, qt.address);
      await P1.makeCoverUsingCA(smartConAdd, toHex('DAI'), coverDetailsDai,
                                coverPeriod, vrsdata[0], vrsdata[1], vrsdata[2],
                                {from : coverHolder});
      coverID = await qd.getAllCoversOfUser(coverHolder);
      await cl.submitClaim(coverID[coverID.length - 1], {from : coverHolder});
  ...
  describe('More basic test cases', function() {
    before(async function() {
      ...
      await tc.lock(CLA, ether(200), duration.days(300), {from : member5});
    ...
    it('8.31 should not allow to cast CA vote if not locked under CA',
       async function() {
         let clid = (await cd.actualClaimLength()) - 1;
         await assertRevert(cl.submitCAVote(clid, -1, {from : staker1}));
       });
    it('8.32 should not be able to CAs right to cast CA vote for 3 days',
       async function() {
         await assertRevert(cd.setUserClaimVotePausedOn(member1));
       });
    it('8.33 should not be able to update uint parameter directly',
       async function() {
    ...
    it('8.35 even if passed by governance should not trigger action for wrong code',
       async function() {
         actionHash = encode('updateUintParameters(bytes8,uint)', 'asd', 12);
         await gvProp(24, actionHash, mr, gv, 2);
       });
    it('8.36 should able to propose to block CAs right to cast CA vote for 3 days',
       async function() {
    ...
    it('8.39 should close voting during casting vote as Member',
       async function() {
         coverID = await qd.getAllCoversOfUser(coverHolder);
         await cl.submitClaim(coverID[coverID.length - 1],
                              {from : coverHolder});
         let clid = (await cd.actualClaimLength()) - 1;
         let now = await latestTime();
         let maxVoteTime = await cd.maxVotingTime();
    ...
    it('8.41 should handle if commissionToBePaid is 0',
       async function() { await P1.updateStakerCommissions(smartConAdd, 0); });
