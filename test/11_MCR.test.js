...
contract('MCR', function([ owner, notOwner ]) {
  ...
  describe('Initial MCR cap test cases', function() {
    it('Testing new threshold condition with standard values',
       async function() {
         let thresholdValues = await mcr.getThresholdValues(
             new BN(toWei(7072).toString()), new BN(toWei(7060).toString()),
             new BN((2218).toString()), new BN((7).toString()));
         thresholdValues[0].toString().should.be.equal(new BN(9184).toString());
         thresholdValues[1].toString().should.be.equal(
             new BN(12123).toString());
         let thresholdValues1 = await mcr.getThresholdValues(
             new BN(toWei(7072).toString()), new BN(toWei(7060).toString()),
             new BN((20000).toString()), new BN((7).toString()));
         thresholdValues1[0].toString().should.be.equal(
             new BN(7072).toString());
         thresholdValues1[1].toString().should.be.equal(
             new BN(12123).toString());
         thresholdValues2 = await mcr.getThresholdValues(
             new BN('9127095013938829399629'.toString()),
             new BN(toWei(9127).toString()), new BN((4856).toString()),
             new BN((7).toString()));
         thresholdValues2[0].toString().should.be.equal(
             new BN(11853).toString());
         thresholdValues2[1].toString().should.be.equal(
             new BN(15646).toString());
       });
    it('11.1 post mcr before launch should not affect initialMCRCap',
       async function() {
         let cap = await pd.capReached();
         await mcr.addMCRData(18000, toWei(100), toWei(2),
                              [ '0x455448', '0x444149' ], [ 100, 65407 ],
                              20181011);
    ...
    describe('After launch', function() {
      ...
      it('11.2 After launch cap should not be set until it reached 100 for 1st time',
         async function() {
           await mcr.addMCRData(1800, toWei(100), toWei(2),
                                [ '0x455448', '0x444149' ], [ 100, 65407 ],
                                20181011);
           ((await mcr.variableMincap()) / 1e18)
               .toString()
      ...
      it('11.4 After launch cap should be set to 1 if reached 100% for 1st time on 30th day',
         async function() {
           await mcr.addMCRData(18000, toWei(100), toWei(2),
                                [ '0x455448', '0x444149' ], [ 100, 65407 ],
                                20181011);
  ...
  describe('Calculation of V(tp) and MCR(tp)', function() {
    ...
    before(async function() {
      ...
      balance_ETH =
          new BN(balance_ETH.toString())
              .add(new BN((await p1.getInvestmentAssetBalance()).toString()));
    });
    ...
    it('11.5 should return correct V(tp) price', async function() {
      ...
      cal_vtp.toString().should.be.equal(
          (await mcr.calVtpAndMCRtp())[0].toString());
    });
    ...
    it('11.6 should return correct MCR(tp) price', async function() {
      ...
      cal_mcrtp.toString().should.be.equal(
          (await mcr.calVtpAndMCRtp())[1].toString());
    });
  });
  ...
  describe('Token Price Calculation', function() {
    ...
    before(async function() {
      ...
      const mcrEth = new BN((await pd.getLastMCREther()).toString())
                         .div(new BN(toWei(1).toString()));
      const tp =
          sf + (parseFloat(mcrEth.toString()) / parseFloat(C.toString())) *
  ...
  describe('Misc', function() {
    ...
    it('11.16 should not be able to add mcr data if not notarise',
       async function() {
         await assertRevert(mcr.addMCRData(
             18000, toWei(100), toWei(2), [ '0x455448', '0x444149' ],
             [ 100, 65407 ], 20181011, {from : notOwner}));
       });
    it('11.17 add mcr when vf > vtp', async function() {
      await mcr.addMCRData(18000, toWei(100), toWei(35.83333333333333),
    ...
    it('11.18 getAllSumAssurance function should skip calcualation for currency with rate 0',
       async function() {
         await DSV.setRate(0);
    ...
    it('11.19 calVtpAndMCRtp function should skip calcualation for currency with rate 0',
       async function() {
         let vtp = await mcr.calVtpAndMCRtp();
         CABalE = await web3.eth.getBalance(p1.address);
         CABalE2 = await web3.eth.getBalance(p2.address);
         vtp[0].toString().should.be.equal(new BN(CABalE.toString())
                                               .add(new BN(CABalE2.toString()))
                                               .toString());
       });
    it('11.20 mcrTp should be 0 if vFull is 0', async function() {
      await mcr.addMCRData(await getValue(0, pd, mcr), toWei(100), 0,
                           [ '0x455448', '0x444149' ], [ 100, 65407 ], 20181011,
                           {from : owner});
      ((await mcr.variableMincap()) / 1e18)
          .toString()
    ...
    it('11.21 mcr if vtp is 0', async function() {
      ...
      await mcr.addMCRData(18000, toWei(100), 0, [ '0x455448', '0x444149' ],
                           [ 100, 65407 ], 20181011, {from : owner});
      ((await mcr.variableMincap()) / 1e18)
          .toString()
          .should.be.equal((357.0703507).toString());
      let APIID = await pd.allAPIcall((await pd.getApilCallLength()) - 1);
      let timeINC = (await pd.getDateAddOfAPI(APIID)) / 1 +
                    (await pd.mcrFailTime()) / 1 + 100;
    ...
    it('11.22 rebalancing trade if total risk balance is 0', async function() {
      ...
      await p2.saveIADetails([ '0x455448', '0x444149' ], [ 100, 15517 ],
                             20190103, true);
    });
    it('11.23 if mcr fails and retry after new mcr posted', async function() {
      ...
      await p1.upgradeInvestmentPool(cad.address);
      await mcr.addMCRData(18000, toWei(100), 0, [ '0x455448', '0x444149' ],
                           [ 100, 65407 ], 20181012, {from : owner});
      ((await mcr.variableMincap()) / 1e18)
          .toString()
          .should.be.equal((430.64105420699997).toString());
      let APIID = await pd.allAPIcall((await pd.getApilCallLength()) - 1);
      await mcr.addMCRData(18000, toWei(100), toWei(100),
                           [ '0x455448', '0x444149' ], [ 100, 65407 ], 20181013,
                           {from : owner});
      ((await mcr.variableMincap()) / 1e18)
          .toString()
    ...
    it('should not be able to update capital model parameters directly',
       async function() {
