...
contract('NXMaster: Emergency Pause', function([
           owner, member1, member2, member3, member4, member5, coverHolder1,
           coverHolder2, coverHolder3, newMember
         ]) {
  ...
  describe('Before Emergency Pause', function() {
    before(async function() {
      ...
      vrsdata = await getQuoteValues(coverDetails, toHex('ETH'), coverPeriod,
      ...
      await tc.lock(CLA, ether(60), validity, {from : member1});
      await tc.lock(CLA, ether(9), validity, {from : member2});
      let proposalsIDs = [];
      await cr.claimAllPendingReward(20, {from : member4});
    });
    it('10.1 should return false for isPause',
       async function() { (await nxms.isPause()).should.equal(false); });
    ...
    it('10.3 should be able to do claim assessment or stake NXM for claim',
       async function() {
    ...
    it('10.6 should be able to withdraw membership',
       async function() { await mr.withdrawMembership({from : member4}); });
  });
  ...
  describe('Emergency Pause: Active', function() {
    ...
    before(async function() {
      ...
      await qt.initiateMembershipAndCover(
          ...
          vrsdata[1], vrsdata[2], {from : newMember, value : totalFee});

      let p = await gv.getProposalLength();
      await gv.createProposalwithSolution(
          'Implement Emergency Pause', 'Implement Emergency Pause',
          'Implement Emergency Pause', 6, 'Implement Emergency Pause',
          '0x872f1eb3');
      ...
      await assertRevert(qt.initiateMembershipAndCover(
          smartConAdd, toHex('ETH'), coverDetails, coverPeriod, v, r, s,
          {from : newMember, value : totalFee}));
    });
    it('10.7 should return true for isPause',
       async function() { (await nxms.isPause()).should.equal(true); });
    ...
    it('10.10 should not be able to trigger kyc',
       async function() { await assertRevert(mr.kycVerdict(member5, true)); });
    ...
    it('10.12 should not let member vote for claim assessment',
       async function() {
         const claimId = (await cd.actualClaimLength()) - 1;
    ...
    it('10.17 should not be able to do claim assessment or stake NXM for claim',
       async function() {
         await assertRevert(
  ...
  describe('Emergency Pause: Inactive', function() {
    before(async function() {
      ...
      let p = await gv.getProposalLength();
      await gv.createProposal('close Emergency Pause', 'close Emergency Pause',
                              'close Emergency Pause', 0);
      await gv.categorizeProposal(p.toNumber(), 7, 0);
      await gv.submitProposalWithSolution(
          p, 'Implement Emergency Pause',
      ...
      for (iteration = 0; iteration < members[1].length; iteration++)
        await gv.submitVote(p, 1, {from : members[1][iteration]});
    ...
    describe('Resume Everything', function() {
      it('10.27 should return false for isPause',
         async function() { (await nxms.isPause()).should.equal(false); });
