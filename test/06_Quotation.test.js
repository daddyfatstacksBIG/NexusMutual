...
const coverDetails =
    [ 1, '3362445813369838', '744892736679184', '7972408607', '7972408607000' ];
const coverDetailsLess =
    [ 5, '19671964915000000', '20000000000000000000', '3549627424' ];
const coverDetailsDai =
    [ 5, '16812229066849188', '5694231991898', '7972408607' ];
const vrs = [
  28, '0x66049184fb1cf394862cca6c3b2a0c462401a671d0f2b20597d121e56768f90a',
  '0x4c28c8f8ff0548dd3a41d7c75621940eb4adbac13696a2796e98a59691bf53ff'
];
const vrsLess = [
  27, '0x22d150b6e2d3f9ae98c67425d1224c87aed5f853487252875118352771b3ece2',
  '0x0fb3f18fc2b8a74083b3cf8ca24bcf877a397836bd4fa1aba4c3ae96ca92873b'
];
const vrs_dai = [
  27, '0xdcaa177410672d90890f1c0a42a965b3af9026c04caedbce9731cb43827e8556',
...
contract('Quotation', function([
           owner, member1, member2, member3, member4, member5, notMember,
           newMember1, newMember2, newMember3, newMember4, newMember5,
           newMember6
         ]) {
  ...
  describe('Initial cap not reached', function() {
    it('6.1 should revert while buying cover', async function() {
      await mr.payJoiningFee(newMember6, {from : newMember6, value : fee});
      await mr.kycVerdict(newMember6, true);
      await assertRevert(P1.makeCoverBegin(
          smartConAdd, toHex('ETH'), coverDetails, coverPeriod, vrs[0], vrs[1],
          vrs[2], {from : newMember6, value : coverDetails[1]}));
    });

    it('6.2 should return 1 if 100% mcr reached within 30 days of launch',
       async function() {
         await mcr.addMCRData(await getValue(toWei(2), pd, mcr), toWei(100),
                              toWei(2), [ '0x455448', '0x444149' ],
                              [ 100, 65407 ], 20181011);
  ...
  describe('Cover Purchase', function() {
    describe('Details', function() {
      it('6.3 should return correct AuthQuoteEngine address', async function() {
        ...
        authQE.toString().should.equal(
            web3.utils.toChecksumAddress(QE).toString());
      });
    ...
    describe('If user is a member', function() {
      ...
      describe('If user does not have sufficient funds', function() {
        it('6.5 reverts', async function() {
          await assertRevert(P1.makeCoverBegin(
              smartConAdd, toHex('ETH'), coverDetails, coverPeriod, vrs[0],
              vrs[1], vrs[2], {from : member1, value : coverDetails[1] - 1}));
          await assertRevert(qt.makeCoverUsingNXMTokens(
              coverDetails, coverPeriod, toHex('ETH'), smartConAdd, vrs[0],
              vrs[1], vrs[2], {from : member1}));
          await assertRevert(P1.makeCoverUsingCA(
              smartConAdd, toHex('DAI'), coverDetailsDai, coverPeriod,
              vrs_dai[0], vrs_dai[1], vrs_dai[2], {from : member1}));
        });
      });
      ...
      describe('If user does have sufficient funds', function() {
        describe('If staker not staked tokens on Smart Contract', function() {
          describe('Purchase Cover With Ether', function() {
            ...
            it('6.8 should not be able to purchase cover if premiumNXM is 0',
               async function() {
                 ...
                 coverDetails[2] = 0;
                 await assertRevert(P1.makeCoverBegin(
                     smartConAdd, toHex('ETH'), coverDetails, coverPeriod,
                     vrs[0], vrs[1], vrs[2],
            ...
            it('6.9 should be able to purchase cover', async function() {
              ...
              const initialTokensOfCoverHolder =
                  await tk.balanceOf(coverHolder);
              initialTotalSupply = (await tk.totalSupply()).div(P_18);
              var vrsdata =
                  await getQuoteValues(coverDetails, toHex('ETH'), coverPeriod,
                                       smartConAdd, qt.address);
              await P1.makeCoverBegin(
                  smartConAdd, toHex('ETH'), coverDetails, coverPeriod,
                  vrsdata[0], vrsdata[1], vrsdata[2],
                  {from : coverHolder, value : coverDetails[1]});
              const newLockedCN =
                  BN_10.mul(new BN(coverDetails[2].toString())).div(BN_100);
              const newPoolBalance =
                  new BN(initialPoolBalance.toString())
                      .add(new BN(coverDetails[1].toString()));
              const newTotalSA = new BN(coverDetails[0].toString());
              const newTotalSupply =
                  new BN(initialTotalSupply.toString())
                      .add(new BN(newLockedCN.toString())
                               .div(new BN(P_18.toString())));
              newLockedCN.toString().should.be.equal(
                  (await tf.getUserLockedCNTokens.call(coverHolder, 1))
                      .toString());
              newPoolBalance.toString().should.be.equal(
                  (await web3.eth.getBalance(P1.address)).toString());
              newTotalSA.toString().should.be.equal(
            ...
            it('6.10 should be revert if smart contract address is null',
               async function() {
                 coverDetails[4] = 7972408607114;
                 var vrsdata =
                     await getQuoteValues(coverDetails, toHex('ETH'),
                                          coverPeriod, smartConAdd, qt.address);
                 await assertRevert(P1.makeCoverBegin(
                     nullAddress, toHex('ETH'), coverDetails, coverPeriod,
                     vrsdata[0], vrsdata[1], vrsdata[2],
                     {from : coverHolder, value : coverDetails[1]}));
               });
            it('6.11 should return correct cover details', async function() {
              ...
              let smartCACompare = web3.utils.toChecksumAddress(cdetails1[2]) ==
                                   web3.utils.toChecksumAddress(smartConAdd);
              if (cdetails1[3] == CA_ETH && cdetails1[1] == coverHolder &&
                  smartCACompare) {
                checkd = true;
              }
          ...
          describe('Purchase Cover With NXM', function() {
            ...
            it('6.12 should not have locked Cover Note initially',
               async function() {
                 const initialLockedCN =
                     await tf.getUserAllLockedCNTokens.call(coverHolder);
                 initialLockedCN.toString().should.be.equal((0).toString());
               });
            it('6.13 total sum assured should be 1 ETH initially',
               async function() {
            ...
            it('6.14 should be able to purchase cover', async function() {
              ...
              await qt.makeCoverUsingNXMTokens(
                  coverDetails, coverPeriod, toHex('ETH'), smartConAdd,
                  vrsdata[0], vrsdata[1], vrsdata[2], {from : coverHolder});
              const newLockedCN = new BN(BN_10.toString())
                                      .mul(new BN(coverDetails[2].toString()))
                                      .div(new BN(BN_100.toString()));
              const newTotalSA = new BN(initialTotalSA.toString())
                                     .add(new BN(coverDetails[0].toString()));
              const newTokensOfCoverHolder =
                  new BN(initialTokensOfCoverHolder.toString())
              ...
              newTotalSA.toString().should.be.equal(
                  (await qd.getTotalSumAssured(CA_ETH)).toString());
              newTokensOfCoverHolder.toString().should.be.equal(
                  (await tk.balanceOf(coverHolder)).toString());
              newTotalSupply.toString().should.be.equal(
                  new BN((await tk.totalSupply()).toString())
                      .div(new BN(P_18.toString()))
          ...
          describe('Purchase Cover With DAI', function() {
            ...
            before(async function() {
              await mr.payJoiningFee(coverHolder,
                                     {from : coverHolder, value : fee});
              await mr.kycVerdict(coverHolder, true);
              await P1.buyToken({from : coverHolder, value : tokenAmount});
              await tk.approve(tc.address, UNLIMITED_ALLOWANCE,
                               {from : coverHolder});
              await cad.transfer(coverHolder, tokenDai);
            });
            it('6.16 should not have locked Cover Note initially',
               async function() {
                 const initialLockedCN =
                     await tf.getUserAllLockedCNTokens.call(coverHolder);
                 initialLockedCN.toString().should.be.equal((0).toString());
               });
            it('6.17 total sum assured should be 2 ETH initially',
               async function() {
                 initialTotalSA = await qd.getTotalSumAssured(CA_ETH);
                 initialTotalSA.toString().should.be.equal((2).toString());
               });
            it('6.18 should able to purchase cover using currency assest i.e. DAI ',
               async function() {
                 ...
                 var vrsdata =
                     await getQuoteValues(coverDetailsDai, toHex('DAI'),
                                          coverPeriod, smartConAdd, qt.address);
                 ...
                 newTotalSupplyVal = newTotalSupplyVal.substring(
                     0, newTotalSupplyVal.length - 2);
                 var presentTotalSupplyVal = presentTotalSupply.toString();
                 var presentTotalSupplyVal = presentTotalSupplyVal.substring(
                     0, presentTotalSupplyVal.length - 2);
                 newTotalSupplyVal.toString().should.be.equal(
                     presentTotalSupplyVal.toString());
               });
            it('6.19 currency assest balance should increase after cover purchase',
               async function() {
                 const presentPoolBalanceOfCA =
                     new BN(coverDetailsDai[1].toString());
            ...
            it('6.20 should return correct cover details purchased with DAI',
               async function() {
                 const CID = await qd.getAllCoversOfUser(coverHolder);
                 let checkd = false;
                 const cdetails1 = await qd.getCoverDetailsByCoverID1(CID[0]);
                 ...
                 let smartCACompare =
                     ...
                     web3.utils.toChecksumAddress(smartConAdd);
                 if (cdetails1[3] == CA_DAI && cdetails1[1] == coverHolder &&
                     smartCACompare) {
                   checkd = true;
                 }
                 checkd.should.equal(true);
        ...
        describe('If staker staked tokens on Smart Contract', function() {
          ...
          describe('Purchase Cover With NXM', function() {
            ...
            it('6.23 should be able to purchase cover', async function() {
              ...
              await assertRevert(qt.makeCoverUsingNXMTokens(
                  newCDetails, coverPeriod, toHex('ETH'), smartConAdd, vrs[0],
                  vrs[1], vrs[2], {from : coverHolder}));
              await assertRevert(qt.makeCoverUsingNXMTokens(
                  coverDetails, coverPeriod, toHex('ETH'), smartConAdd, 27,
                  vrs[1], vrs[2], {from : coverHolder}));
          ...
          describe('Purchase Cover With DAI', function() {
            ...
            let initialStakeCommissionOfS1;
            let initialStakeCommissionOfS2;
            it('6.25 should able to purchase cover using currency assest i.e. DAI ',
               async function() {
                 initialStakeCommissionOfS1 =
                     await td.getStakerTotalEarnedStakeCommission.call(staker1);
                 initialStakeCommissionOfS2 =
            ...
            it('6.27 should able to purchase cover with cover period less than 60 ',
               async function() {
                 ...
                 var vrsdata = await getQuoteValues(
                     coverDetailsLess, toHex('DAI'), coverPeriodLess,
                     smartConAdd, qt.address);
                 await cad.approve(P1.address, coverDetailsLess[1],
                                   {from : coverHolder});
                 await P1.makeCoverUsingCA(smartConAdd, toHex('DAI'),
                                           coverDetailsLess, coverPeriodLess,
                                           vrsdata[0], vrsdata[1], vrsdata[2],
                                           {from : coverHolder});
    ...
    describe('If user is not a member', function() {
      it('6.28 should return -1 if user have no holded Covers',
         async function() {
      ...
      it('6.29 should revert if member', async function() {
        const totalFee =
            new BN(fee.toString()).add(new BN(coverDetails[1].toString()));
        coverDetails[4] = 7972408607214;
        var vrsdata = await getQuoteValues(
            coverDetails, toHex('ETH'), coverPeriod, smartConAdd, qt.address);
        await assertRevert(qt.initiateMembershipAndCover(
            smartConAdd, toHex('ETH'), coverDetails, coverPeriod, vrsdata[0],
            vrsdata[1], vrsdata[2], {from : member1, value : totalFee}));
      });
      describe('if do not want to join membership', function() {
        it('6.30 reverts', async function() {
          await assertRevert(P1.makeCoverBegin(
              smartConAdd, toHex('ETH'), coverDetails, coverPeriod, vrs[0],
              vrs[1], vrs[2], {from : notMember, value : coverDetails[1]}));

          await assertRevert(qt.makeCoverUsingNXMTokens(
              coverDetails, coverPeriod, toHex('ETH'), smartConAdd, vrs[0],
              vrs[1], vrs[2], {from : notMember}));
          await assertRevert(P1.makeCoverUsingCA(
              smartConAdd, toHex('DAI'), coverDetailsDai, coverPeriod,
              vrs_dai[0], vrs_dai[1], vrs_dai[2], {from : notMember}));
          const totalFee =
              new BN(fee.toString()).add(new BN(coverDetails[1].toString()));
          ...
          await assertRevert(qt.initiateMembershipAndCover(
              smartConAdd, toHex('ETH'), coverDetails, coverPeriod, vrsdata[0],
              vrsdata[1], vrsdata[2], {from : notMember, value : 1}));
          coverDetails[4] = 7972408607813;
          var vrsdata = await getQuoteValues(
              coverDetails, toHex('ETH'), coverPeriod, smartConAdd, qt.address);
          await assertRevert(qt.initiateMembershipAndCover(
              smartConAdd, toHex('ETH'), coverDetails, coverPeriod, 10,
              vrsdata[1], vrsdata[2], {from : notMember, value : totalFee}));
          ...
          actionHash =
              encode('upgradeContract(bytes2,address)', 'QT', newQt.address);
          await gvProp(29, actionHash, oldMR, oldGv, 2);
          (await nxms.getLatestAddress(toHex('QT')))
              .should.be.equal(newQt.address);
          qt = newQt;
          await assertRevert(qt.initiateMembershipAndCover(
              smartConAdd, toHex('ETH'), coverDetails, coverPeriod, vrs[0],
              vrs[1], vrs[2], {from : notMember, value : totalFee}));
          let hcl = await qd.getUserHoldedCoverLength(notMember);
          await qt.kycVerdict(notMember, false);
          await mr.payJoiningFee(notMember, {from : notMember, value : fee});
          await mr.kycVerdict(notMember, false);
        });
      });
      describe('if want to join membership', function() {
        it('6.31 should be able to join membership and purchase cover with ETH',
           async function() {
             await tk.approve(tc.address, UNLIMITED_ALLOWANCE,
                              {from : newMember1});
             const totalFee =
        ...
        it('6.32 should be able to join membership and purchase cover with DAI',
           async function() {
             await tk.approve(tc.address, UNLIMITED_ALLOWANCE,
                              {from : newMember2});
        ...
        it('6.34 should revert if wallet address is not set', async function() {
          ...
          let oldGv =
              await Governance.at(await nxms.getLatestAddress(toHex('GV')));
          actionHash = encode('updateOwnerParameters(bytes8,address)',
                              'MSWALLET', nullAddress);
          ...
          await qt.initiateMembershipAndCover(
              smartConAdd, toHex('ETH'), coverDetails, coverPeriod, vrsdata[0],
              vrsdata[1], vrsdata[2], {from : newMember5, value : totalFee});
        ...
        it('6.35 should get membership but not cover if quote expires for ETH',
           async function() {
             // await tk.approve(tc.address, UNLIMITED_ALLOWANCE, {
             //   from: newMember4
             // });
        ...
        it('6.36 should revert if quote validity expires', async function() {
          ...
          const totalFee =
              new BN(fee.toString()).add(new BN(newCoverDetails[1].toString()));
          await assertRevert(qt.initiateMembershipAndCover(
              smartConAdd, toHex('ETH'), newCoverDetails, coverPeriod, vrs[0],
              vrs[1], vrs[2], {from : notMember, value : totalFee}));
        });

        it('6.37 should get membership but not cover if quote expires for DAI',
           async function() {
             await assertRevert(qt.initiateMembershipAndCover(
                 smartConAdd, toHex('DAI'), coverDetailsDai, coverPeriod,
                 vrs_dai[0], vrs_dai[1], vrs_dai[2],
  ...
  describe('Cover Expire', function() {
    ...
    it('6.40 Expired cover should not be expired again',
       async function() { await assertRevert(qt.expireCover(1)); });

    it('6.41 decrease sum assured', async function() {
      const newSumAssured = await qd.getTotalSumAssured(CA_ETH);
      newSumAssured.toString().should.be.equal(
    ...
    it('6.43 should unlock locked cover note tokens', async function() {
      ...
      (await tk.balanceOf(member3))
          .toString()
          .should.be.equal(new BN(initialTokenBalance.toString())
  ...
  describe('Misc', function() {
    describe('Change product params if owner', function() {
      it('6.47 only governance call should be able to change Profit Margin',
         async function() {
           let oldMR =
               await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
           let oldGv =
      ...
      it('6.48 only governance call should be able to change STLP',
         async function() {
           let oldMR =
               await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
           let oldGv =
      ...
      it('6.49 only governance call should be able to change STL',
         async function() {
           let oldMR =
               await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
           let oldGv =
      ...
      it('6.50 only governance call should be able to change minimum cover period',
         async function() {
           let oldMR =
               await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
           let oldGv =
               await Governance.at(await nxms.getLatestAddress(toHex('GV')));
           actionHash =
               encode('updateUintParameters(bytes8,uint)', 'QUOMIND', 4);
           await gvProp(23, actionHash, oldMR, oldGv, 2);
    ...
    describe('if not internal contract address', function() {
      ...
      it('6.52 should not be able to change cover status number',
         async function() {
           const CID = await qd.getAllCoversOfUser(member3);
           await assertRevert(
               qd.changeCoverStatusNo(CID[0], 1, {from : notMember}));
         });
      it('6.53 should fail add mcr if lower threshold not reached',
         async function() {
           await mcr.addMCRData(
               parseInt((await getValue(toWei(2), pd, mcr)) / 2), toWei(100),
               toWei(2), [ '0x455448', '0x444149' ], [ 100, 65407 ], 20181011);
           var APIID = await pd.allAPIcall((await pd.getApilCallLength()) - 1);
           (await pd.getApiIdTypeOf(APIID)).should.be.equal('0x4d435246');
         });
      it('6.54 should throw if call kycVerdict with non authorised address',
         async function() {
           await assertRevert(qt.kycVerdict(member1, true, {from : member1}));
         });
      it('6.55 should not able to update quoatation parameters directly',
         async function() {
