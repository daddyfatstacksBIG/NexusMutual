...
contract('ClaimsReward', function([
           owner, member1, member2, member3, staker1, staker2, coverHolder,
           notMember, newMember1, newMember2
         ]) {
  ...
  describe('Claim Assesor get rewards after Claim Assessment', function() {
    ...
    before(async function() {
      ...
      let claimed1 = await cr.getRewardAndClaimedStatus(1, 0, {from : member1});
    ...
    it('9.2 should increase balance of member', async function() {
      (await tk.balanceOf(member1))
          .toString()
          .should.be.equal(new BN(initialBalance.toString())
  ...
  describe('Staker gets reward', function() {
    ...
    it('9.6 should decrease locked staked tokens of staker', async function() {
      (await tf.getStakerAllLockedTokens(staker1))
          .toString()
          .should.be.equal(new BN(lockedStakedNXM.toString())
                               .sub(new BN(unlockableStakedNXM.toString()))
                               .toString());
    });
    it('9.7 should return zero unlockable staked tokens of staker',
       async function() {
         (await tf.getStakerAllUnlockableStakedTokens(staker1))
             .toString()
             .should.be.equal((0).toString());
  ...
  describe('Misc', function() {
    ...
    it('9.9 should not be able call upgrade function of this contract',
       async function() {
  ...
  describe(
      'Test for claim reward for particular numbers of records', function() {
        let apiidArr = [];
        let conAdds = [];
        before(async function() {
          ...
          let coverDetailsTest = [
            1, '3362445813369838', toWei(100), '7972408607', '7972408607201'
          ];
          for (let i = 0; i < conAdds.length; i++) {
            ...
            var vrsdata =
                await getQuoteValues(coverDetailsTest, toHex('ETH'),
                                     coverPeriod, conAdds[i], qt.address);
            await P1.makeCoverBegin(
                conAdds[i], toHex('ETH'), coverDetailsTest, coverPeriod,
        ...
        it('9.10 should claim commision for covers', async function() {
          assert.equal(
              await td.getStakerTotalReedmedStakeCommission(newMember1), 0);
          let initialLastClaimed =
          ...
          assert.equal(
              ...
              toWei(45));
          assert.equal(await td.lastCompletedStakeCommission(newMember1),
          ...
          assert.equal(await td.lastCompletedStakeCommission(newMember1),
                       initialLastClaimed / 1 + 3);
          let coverDetailsTest = [
            1, '3362445813369838', toWei(100), '7972408607', '7972408607501'
          ];
          coverDetailsTest[2] = toWei(500);
          var vrsdata =
              await getQuoteValues(coverDetailsTest, toHex('ETH'), coverPeriod,
                                   conAdds[3], qt.address);
          await P1.makeCoverBegin(
              conAdds[3], toHex('ETH'), coverDetailsTest, coverPeriod,
              vrsdata[0], vrsdata[1], vrsdata[2],
              {from : newMember2, value : coverDetailsTest[1].toString()});
        ...
        it('9.11 should claim reward for CA votes', async function() {
          ...
          let initialLastClaimed =
              await cd.getRewardDistributedIndex(newMember1);
          ...
          let returnData = await claimAssesmentVoting(
              1, coverID, newMember2, newMember1, cl, cd, pd, P1, '2000');
          let initialBal = await tk.balanceOf(newMember1);
          await cr.claimAllPendingReward(3, {from : newMember1});
          let finalBal = await tk.balanceOf(newMember1);
        ...
        it('9.12 should claim reward for Member votes', async function() {
          ...
          let returnData = await claimAssesmentVoting(
              2, coverID, newMember2, newMember1, cl, cd, pd, P1, '2000');
          let initialBal = await tk.balanceOf(newMember1);
          await cr.claimAllPendingReward(3, {from : newMember1});
          let finalBal = await tk.balanceOf(newMember1);
          assert.equal(parseFloat(returnData[0]), toWei(180));
...
async function claimAssesmentVoting(ca, coverid, newMember2, newMember1, cl, cd,
                                    pd, P1, _increaseTime) {
  let totalOf3;
  let total;
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (ca != 1) {
      ...
      closingTime =
          new BN(_increaseTime.toString()).add(new BN(now.toString()));
      await increaseTimeTo(
          new BN(closingTime.toString()).add(new BN((1).toString())));
