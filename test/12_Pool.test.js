...
contract('Pool', function(
                     [ owner, notOwner, member1, member2, member3, member4 ]) {
  before(async function() {
    ...
    await mcr.addMCRData(18000, toWei(100), toWei(2),
                         [ '0x455448', '0x444149' ], [ 100, 65407 ], 20181011);
  ...
  describe('PoolDataMock', function() {
    describe('Misc', function() {
      it('12.11 should return true if notarise address',
         async function() { (await pd.isnotarise(owner)).should.equal(true); });
    ...
    it('12.16 should not be able to add new Investment Asset',
       async function() {
         await assertRevert(pd.addInvestmentAssetCurrency(
             newAsset, ZERO_ADDRESS, false, 4000, 8500, 18));
       });

    it('12.17 should not be able to change UniswapFactoryAddress directly',
       async function() {
         await assertRevert(
             p2.changeUniswapFactoryAddress(pd.address, {from : notOwner}));
       });

    it('12.29 should not be able to call saveIADetails if not notarise',
       async function() {
         await assertRevert(p2.saveIADetails([ '0x455448', '0x444149' ],
                                             [ 100, 15517 ], 20190103, true,
                                             {from : notOwner}));
       }); // for testing
  ...
  describe('Liquidity', function() {
    it('12.32 Setting the testing parameters', async function() {
      ...
      await p2.saveIADetails([ '0x455448', '0x444149' ], [ 100, 1000 ],
                             20190125, false);
    ...
    it('12.33 ELT ETH (No IA available at IA pool)', async function() {
      ...
      let amount =
          parseFloat(ICABalE) - 1.5 * parseFloat(parseFloat(baseVarMinE[2]) +
                                                 parseFloat(baseVarMinE[1]));
      ...
      await increaseTimeTo((await pd.liquidityTradeCallbackTime()) / 1 +
                           time / 1 + 100);
      ...
      amount =
          parseFloat(CABalD) - 1.5 * parseFloat(parseFloat(baseVarMinE[2]) +
                                                parseFloat(baseVarMinE[1]));
      FCABalE.toString().should.be.equal(CABalE.toString());
      FCABalE2.toString().should.be.equal(
          (amount / ((await pd.getCAAvgRate(toHex('DAI'))) / 100) + CABalE2 * 1)
              .toString());
    ...
    it('12.34 RBT (ETH to ETH)', async function() {
      ...
      await mcr.addMCRData(
          ...
          (ICABalE * 1 + ICABalE2 * 1 + (ICABalD / 10 + ICABalD2 / 10))
              .toString(),
          [ '0x455448', '0x444149' ], [ 100, 1000 ], 20190129);
      await p2.saveIADetails([ '0x455448', '0x444149' ], [ 100, 1000 ],
                             20190129, true);
    ...
    it('12.35 ILT(ETH->ETH)', async function() {
      await pd.changeCurrencyAssetBaseMin(
          '0x455448',
          ((await pd.getCurrencyAssetBaseMin(toHex('ETH'))) * 1 + toWei(5) * 1)
              .toString());
      let ICABalE;
      let ICABalD;
      let ICABalE2;
      ...
      let amount = 1.5 * (parseFloat(baseVarMinE[2]) * 1 +
                          parseFloat(baseVarMinE[1]) * 1) -
                   parseFloat(ICABalE) * 1;
      CABalE.toString().toString().should.be.equal(
          (ICABalE * 1 + amount * 1).toString().toString());
      CABalE2.toString().should.be.equal((ICABalE2 - amount).toString());
      CABalD.toString().should.be.equal(ICABalD.toString());
      CABalD2.toString().should.be.equal(ICABalD2.toString());
    });
    it('12.36 ELT(ETH->DAI)', async function() {
      ...
      await pd.changeCurrencyAssetBaseMin(
          ...
          ((await pd.getCurrencyAssetBaseMin(toHex('ETH'))) * 1 - toWei(5) * 1)
              .toString());
      let baseVarMinE = await pd.getCurrencyAssetVarBase(toHex('ETH'));
      let amount = parseFloat(ICABalE) - 1.5 * (parseFloat(baseVarMinE[2]) +
                                                parseFloat(baseVarMinE[1]));
      await p1.internalLiquiditySwap(toHex('ETH'));
      var APIID = await pd.allAPIcall((await pd.getApilCallLength()) - 1);
      let time = await latestTime();
    ...
    it('12.37 ILT(DAI->DAI)', async function() {
      await pd.changeCurrencyAssetBaseMin(
          toHex('DAI'),
          ((await pd.getCurrencyAssetBaseMin(toHex('DAI'))) * 1 + toWei(5) * 1)
              .toString());
      let ICABalE;
      let ICABalD;
      let ICABalE2;
    ...
    it('12.38 ELT(DAI->DAI)', async function() {
      ...
      ICABalD2 = await cad.balanceOf(p2.address);
      let baseVarMinD = await pd.getCurrencyAssetVarBase(toHex('DAI'));
      let amount = parseFloat(ICABalD) - 1.5 * (parseFloat(baseVarMinD[0]) +
                                                parseFloat(baseVarMinD[1]));
      await p1.internalLiquiditySwap(toHex('DAI'));
      let CABalE;
      let CABalD;
      let CABalE2;
      ...
      await p1.upgradeInvestmentPool(cad.address);
      await p2.sendEther(
          {from : owner, value : (CABalE2 / 1 - toWei(3)).toString()});
      await cad.transfer(p2.address, CABalD2);
      await mcr.addMCRData(
          await getValue((CABalE * 1 + CABalE2 * 1 +
                          (CABalD / 10 + CABalD2 / 10) - toWei(3))
                             .toString(),
                         pd, mcr),
          toWei(100),
          (CABalE * 1 + CABalE2 * 1 + (CABalD / 10 + CABalD2 / 10) - toWei(3))
              .toString(),
          [ '0x455448', '0x444149' ], [ 100, 1000 ], 20190129);
      await p2.saveIADetails([ '0x455448', '0x444149' ], [ 100, 1000 ],
                             20190129, false);
    ...
    it('12.41 ILT DAI to ETH', async function() {
      ...
      let amount = 1.5 * parseFloat(parseFloat(baseVarMinE[2]) +
                                    parseFloat(baseVarMinE[1])) -
                   parseFloat(ICABalD);
      await p1.internalLiquiditySwap(toHex('DAI'));
      var APIID = await pd.allAPIcall((await pd.getApilCallLength()) - 1);
    ...
    it('12.43 ILT ETH to DAI', async function() {
      ...
      CABalD.toString().should.be.equal(ICABalD.toString());
      CABalD2.toString().should.be.equal(
          (ICABalD2 - (amount / 100) * (await pd.getCAAvgRate(toHex('DAI'))))
              .toString());
    });
    ...
    it('12.45 Initial ELT(ETH->DAI) but at time of call back ELT(ETH->ETH)',
       async function() {
         let ICABalE;
         let ICABalD;
         let ICABalE2;
         ...
         await p1.upgradeInvestmentPool(cad.address);
         await p2.sendEther(
             {from : owner, value : (p2Eth / 1 - toWei(5)).toString()});
    ...
    it('12.46 ELT(ETH->DAI) amount > price slippage', async function() {
      ...
      (CABalD2 / toWei(1))
          .toFixed(0)
          .toString()
          .should.be.equal(((ICABalD2 * 1 + toWei(4.8) * 1) / toWei(1))
                               .toFixed(0)
                               .toString());
    });
    ...
    it('12.48 ILT(ETH->DAI) amount > price slippage', async function() {
      ...
      (CABalE / toWei(1))
          .toFixed(0)
          .toString()
          .should.be.equal(((ICABalE * 1 + toWei(0.48) * 1) / toWei(1))
                               .toFixed(0)
                               .toString());
      CABalE2.toString().should.be.equal(ICABalE2.toString());
      CABalD.toString().should.be.equal(ICABalD.toString());
    ...
    it('12.49 ILT(DAI->ETH) amount > price slippage', async function() {
      ...
      await p1.sendEther({
        from : owner,
        value :
            (toWei(6) * 1 - (await web3.eth.getBalance(p1.address))).toString()
      });
      await tf.transferCurrencyAsset(toHex('DAI'), owner, toWei(5));
      await p2.saveIADetails([ '0x455448', '0x444149' ], [ 100, 1000 ],
                             20190310, false);
      ...
      (CABalE2 / toWei(1))
          .toFixed(0)
          .toString()
          .should.be.equal(((ICABalE2 * 1 - toWei(0.4) * 1) / toWei(1))
                               .toFixed(0)
                               .toString());
      (CABalD / toWei(1))
          .toFixed(0)
    ...
    it('12.50 ILT(ETH->DAI) IA dont have enough amount', async function() {
      ...
      (CABalE / toWei(1))
          .toFixed(0)
          .toString()
          .should.be.equal(((ICABalE * 1 + toWei(7.06) * 1) / toWei(1))
                               .toFixed(0)
                               .toString());
      CABalE2.toString().should.be.equal(ICABalE2.toString());
      CABalD.toString().should.be.equal(ICABalD.toString());
    ...
    it('12.51 ILT(DAI->ETH) IA dont have enough amount', async function() {
      await p1.upgradeInvestmentPool(cad.address);
      await p2.sendEther(
          {from : owner, value : (CABalE2 / 1 - toWei(5)).toString()});
      await cad.transfer(p2.address, CABalD2);
      ...
      let checkCondition =
          ...
          parseFloat(((ICABalE2 * 1 - toWei(1.14) * 1) / toWei(1))
                         .toFixed(0)
                         .toString());
      ...
      ((CABalD * 1) / toWei(1))
          .toFixed(0)
          .toString()
          .should.be.equal(((ICABalD * 1 + toWei(11.4) * 1) / toWei(1))
                               .toFixed(0)
                               .toString());
      (CABalD2 / toWei(1))
          .toFixed(0)
    ...
    it('12.53 Initial ILT(DAI->ETH) but at time of call back ILT(DAI->DAI)',
       async function() {
         await p2.sendEther({from : owner, value : toWei(5)});
         await p2.saveIADetails([ '0x455448', '0x444149' ], [ 100, 1000 ],
                                20190311, false);
         ...
         (CABalD2 / toWei(1))
             .toFixed(0)
             .toString()
             .should.be.equal(((ICABalD2 * 1 - amount / 1) / toWei(1))
                                  .toFixed(0)
                                  .toString());
       });
  });
  describe('Should be able to delegate callback for', function() {
    it('12.54 Expire Cover ', async function() {
      ...
      await cad.approve(p1.address, coverDetailsLess[1], {from : member1});
      await cad.transfer(member1, tokenDai);
      coverDetailsLess[4] = 7972408607001;
      var vrsdata =
          await getQuoteValues(coverDetailsLess, toHex('DAI'), coverPeriodLess,
                               smartConAdd, qt.address);
      await p1.makeCoverUsingCA(smartConAdd, toHex('DAI'), coverDetailsLess,
                                coverPeriodLess, vrsdata[0], vrsdata[1],
                                vrsdata[2], {from : member1});
  ...
  describe('Trade Conditions checked', function() {
    it('12.56 For iaRate = 0', async function() {
      await p2.saveIADetails([ '0x455448', '0x444149' ], [ 0, 0 ], 20190125,
                             true);
    });
  });
  describe('Liquidity trade Token to Token', function() {
    before(async function() {
      ...
      let actionHash = encode(
          ...
          '0x4d4b52', mkr.address, true, 500, 5000, 18);
      ...
      await p2.saveIADetails([ '0x455448', '0x444149', '0x4d4b52' ],
                             [ 100, 1000, 500 ], 20190311, false);
    ...
    it('12.57 ELT(DAI->MKR)', async function() {
      ...
      let amount =
          parseFloat(ICABalD) - 1.5 * parseFloat(parseFloat(baseVarMinE[2]) +
                                                 parseFloat(baseVarMinE[1]));
      await p1.internalLiquiditySwap(toHex('DAI'));
      var APIID = await pd.allAPIcall((await pd.getApilCallLength()) - 1);
      let time = await latestTime();
      ...
      (CABalM / toWei(1))
          .toFixed(0)
          .toString()
          .should.be.equal(((ICABalM * 1 +
                             amount / ((await pd.getIAAvgRate(toHex('DAI'))) /
    ...
    it('12.61 ELT(DAI->MKR) amount > price slippage', async function() {
      ...
      await p1.__callback(
          APIID,
          ''); // to cover else branch (if call comes before callback time)
      time = await latestTime();
      await increaseTimeTo((await pd.liquidityTradeCallbackTime()) / 1 +
                           time / 1 + 100);
      await p1.__callback(APIID, '');
    ...
    it('12.63 ILT(DAI->DAI) IA dont have sufficeint ETH', async function() {
      ...
      (CABalD / toWei(1))
          .toFixed(0)
          .toString()
          .should.be.equal(((ICABalD / 1 + toWei(17.9) * 1) / toWei(1))
                               .toFixed(0)
                               .toString());
      let checkCondition =
  ...
  describe('More basic cases', function() {
    it('12.64 RBT For 0 balance in risk pool', async function() {
      ...
      await tf.upgradeCapitalPool(cad.address);
      await tf.upgradeCapitalPool(cad.address);
      await mcr.addMCRData(await getValue(0, pd, mcr), 0, 0,
                           [ '0x455448', '0x444149' ], [ 100, 1000 ], 20190129);
      let pId = (await gv.getProposalLength()).toNumber();
      await gv.createProposal('Inactive DAI', 'Inactive DAI', 'Inactive DAI', 0,
                              {from : member1});
      await gv.categorizeProposal(pId, 14, 0);
      let actionHash =
          encode('changeInvestmentAssetStatus(bytes4,bool)', '0x444149', false);
      ...
      await p2.saveIADetails([ '0x444149', '0x455448' ], [ 100, 15517 ],
                             20190103, false);
    });

    it('12.65 TransferEther should revert when called by other than govern',
       async function() {
         await assertRevert(p1.transferEther(toWei(1), owner));
       });
    it('12.66 should able to propose change in holding percentages',
       async function() {
         let pId = (await gv.getProposalLength()).toNumber();
         await gv.createProposal('change holding perc', 'change holding perc',
                                 'change holding perc', 0, {from : member1});
         await gv.categorizeProposal(pId, 13, 0);
         let actionHash =
             encode('changeInvestmentAssetHoldingPerc(bytes4,uint64,uint64)',
                    '0x444149', 100, 1000);
         await gv.submitProposalWithSolution(pId, 'change holding perc',
                                             actionHash, {from : member1});
         await gv.submitVote(pId, 1, {from : member1});
         await gv.submitVote(pId, 1, {from : member2});
         await gv.submitVote(pId, 1, {from : member3});
         await gv.submitVote(pId, 1, {from : member4});
    ...
    it('12.67 should not be able to change holding percentages directly',
       async function() {
         let initialPerc = await pd.getInvestmentAssetHoldingPerc(toHex('DAI'));
         await assertRevert(
    ...
    it('12.68 should able to propose new currency asset', async function() {
      ...
      let actionHash =
          encode('addCurrencyAssetCurrency(bytes4,address,uint)', toHex('MKR'),
                 mkr.address, '10000000000000000000');
      ...
      (await pd.getCurrencyAssetAddress(toHex('MKR')))
          .should.be.equal(mkr.address);
    });
    it('12.69 should not be able to add new currency asset directly',
       async function() {
         await assertRevert(
             pd.addCurrencyAssetCurrency('0x49434e', mkr.address, toWei(11)));
       });
    it('12.70 should not be able to change IA status directly',
       async function() {
         await assertRevert(pd.changeInvestmentAssetStatus('0x49434e', false));
       });
    it('12.71 should not be able to update pool parameters directly',
       async function() {
         await assertRevert(pd.updateUintParameters('0x49434e', 12));
       });
    it('12.72 should be able to propose new currency address by owner',
       async function() {
         let actionHash = encode('changeCurrencyAssetAddress(bytes4,address)',
                                 toHex('DAI'), member4);
         await gvProp(30, actionHash, mr, gv, 3);
         (await pd.getCurrencyAssetAddress(toHex('DAI')))
             .should.be.equal(member4);
       });
    it('12.73 should be able to propose new IA address and decimal by owner',
       async function() {
         let actionHash = encode(
             'changeInvestmentAssetAddressAndDecimal(bytes4,address,uint8)',
             toHex('DAI'), member3, 16);
         await gvProp(32, actionHash, mr, gv, 3);
         (await pd.getInvestmentAssetAddress(toHex('DAI')))
             .should.be.equal(member3);
         ((await pd.getInvestmentAssetDecimals(toHex('DAI'))) /
          1).should.be.equal(16);
       });
  });
});
