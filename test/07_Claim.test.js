...
contract('Claim', function([
           owner, member1, member2, member3, member4, notCoverHolder, notMember,
           govVoter1, govVoter2, govVoter3, govVoter4
         ]) {
    ...
    describe('if member', function() {
      ...
      describe('if holds a cover', function() {
        before(async function() {
          ...
          vrsdata = await getQuoteValues(coverDetails, toHex('ETH'),
          ...
          describe('if cover does not expires', function() {
            describe('if claim is not submitted yet', function() {
              ...
              it('7.2 should be able to submit claim', async function() {
                ...
                initialCurrencyAssetVarMin =
                    await pd.getCurrencyAssetVarMin(coverCurr);
                await P1.transferFundToOtherAdd(
                    owner, toWei(5)); // To check insufficientTrade condition

                let initialUserClaimCount =
              ...
              it('7.3 cover status should change', async function() {
                ...
                claimDetails[0].toString().should.be.equal(
                    coverID[0].toString());
              ...
              it('7.4 should increase CurrencyAssetVarMin', async function() {
                ...
                // const sumAssured1 =sumAssured.plus(await
                // qd.getCoverSumAssured(coverID[1]));
                (await pd.getCurrencyAssetVarMin(coverCurr))
                    .toString()
                    .should.be.equal(
                        new BN(initialCurrencyAssetVarMin.toString())
                            .add(new BN(sumAssured.toString())
                                     .mul(new BN(ether(1).toString())))
                            .toString());
              });
            });
      ...
      it('7.17 should be able to propose new max voting Time',
         async function() {
           ...
           let oldGv =
               await Governance.at(await nxms.getLatestAddress(toHex('GV')));
           let actionHash =
               encode('updateUintParameters(bytes8,uint)', 'CAMAXVT', 10);
           await gvProp(24, actionHash, oldMR, oldGv, 2);
           let val = await cd.getUintParameters(toHex('CAMAXVT'));
           (val[1] / 1).should.be.equal(10);
         });
      it('7.18 should be able to propose new Payout retry time',
         async function() {
           let oldMR =
               await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
           let oldGv =
               await Governance.at(await nxms.getLatestAddress(toHex('GV')));
           let actionHash =
               encode('updateUintParameters(bytes8,uint)', 'CAPRETRY', 120);
           await gvProp(24, actionHash, oldMR, oldGv, 2);
           let val = await cd.getUintParameters(toHex('CAPRETRY'));
           (val[1] / 1).should.be.equal(120);
         });
      it('7.21 should be able to propose new claim deposit time',
         async function() {
           let oldMR =
               await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
           let oldGv =
               await Governance.at(await nxms.getLatestAddress(toHex('GV')));
           let actionHash =
               encode('updateUintParameters(bytes8,uint)', 'CADEPT', 12);
           await gvProp(24, actionHash, oldMR, oldGv, 2);
           let val = await cd.getUintParameters(toHex('CADEPT'));
           (val[1] / 1).should.be.equal(12);
         });

      it('7.22 should be able to propose new claim reward percentage',
         async function() {
           let oldMR =
               await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
           let oldGv =
               await Governance.at(await nxms.getLatestAddress(toHex('GV')));
           let actionHash =
               encode('updateUintParameters(bytes8,uint)', 'CAREWPER', 36);
           await gvProp(24, actionHash, oldMR, oldGv, 2);
           ((await cd.claimRewardPerc()) / 1).should.be.equal(36);
         });
      it('7.23 should revert if trying to update pendingClaimStart with low value',
         async function() {
           await assertRevert(
