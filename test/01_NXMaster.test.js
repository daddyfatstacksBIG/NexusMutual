...
contract('NXMaster', function([
           owner, newOwner, member, nonMember, anotherAccount, govVoter1,
           govVoter2, govVoter3, govVoter4
         ]) {
  ...
  before(async function() {
    ...
    for (let itr = 5; itr < 9; itr++) {
      await oldMR.payJoiningFee(web3.eth.accounts[itr],
                                {from : web3.eth.accounts[itr], value : fee});
  ...
  describe('Updating state', function() {
    it('1.2 should not be able to change master address if master has not initialized',
       async function() {
         let newMaster = await NXMaster.new(nxmtk.address);
         await assertRevert(nxms.changeMasterAddress(newMaster.address));
       });

    it('1.3 should be able to change single contract (proxy contracts)',
       async function() {
         this.timeout(0);
         let newMemberRoles = await MemberRoles.new();
         let actionHash =
             encode('upgradeContractImplementation(bytes2,address)', 'MR',
         ...
         (await qd.getImplementationAdd(toHex('MR')))
             .should.be.equal(newMemberRoles.address);
         memberRoles = newMemberRoles;
         memberRoles =
             await MemberRoles.at(await nxms.getLatestAddress('0x4d52'));
       });

    it('1.4 should set launch bit after adding initial members',
       async function() {
         memberRoles =
             await MemberRoles.at(await nxms.getLatestAddress('0x4d52'));
    ...
    it('1.5 should be able to reinitialize', async function() {
      ...
      let actionHash =
          encode('updateOwnerParameters(bytes8,address)', 'OWNER', nonMember);
      ...
      actionHash =
          encode('updateOwnerParameters(bytes8,address)', 'OWNER', owner);
      await gvProp(28, actionHash, oldMR, oldGv, 3);
      (await nxms.owner()).should.be.equal(owner);
      actionHash =
          encode('updateOwnerParameters(bytes8,address)', 'QUOAUTH', owner);
      await gvProp(28, actionHash, oldMR, oldGv, 3);
      (await qd.authQuoteEngine()).should.be.equal(owner);
      actionHash =
          encode('updateOwnerParameters(bytes8,address)', 'QUOAUTH', QE);
      await gvProp(28, actionHash, oldMR, oldGv, 3);
      ...
      actionHash =
          encode('updateOwnerParameters(bytes8,address)', 'MCRNOTA', QE);
      await gvProp(28, actionHash, oldMR, oldGv, 3);
      (await pd.notariseMCR()).should.be.equal(qeAdd2);

      actionHash =
          encode('updateOwnerParameters(bytes8,address)', 'MCRNOTA', owner);
      await gvProp(28, actionHash, oldMR, oldGv, 3);
      (await pd.notariseMCR()).should.be.equal(owner);
      await mcr.addMCRData(await getValue(toWei(2), pd, mcr), toWei(100),
                           toWei(2), [ '0x455448', '0x444149' ], [ 100, 15517 ],
                           20190103);
    ...
    // it('1.6 should be able to change token controller address', async
    // function() {
    //   await tc.changeOperator(tc.address);
    // });

    // it('1.8 new Owner should be able to change owner address back to original
  ...
  describe('modifiers', function() {
    ...
    it('1.14 should return true if internal contract address',
       async function() {
         const isInternal = await nxms.isInternal(nxms.address);
         isInternal.should.equal(true);
       });
    it('1.15 should return false if not internal contract address',
       async function() {
  ...
  describe('emergency pause ', function() {
    ...
    it('1.22 other address/contract should not be able to update pauseTime',
       async function() {
    ...
    it('1.23 governance call should be able to update pauseTime',
       async function() {
         let oldMR =
             await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
         let oldGv =
  ...
  describe('upgrade single non-proxy contracts', function() {
    it('1.24 should able to propose new contract code for quotation',
       async function() {
    ...
    it('1.25 should able to propose new contract code for claimsReward',
       async function() {
         ...
         let oldGv =
             await Governance.at(await nxms.getLatestAddress(toHex('GV')));
         actionHash =
             encode('upgradeContract(bytes2,address)', 'CR', newCr.address);
         await gvProp(29, actionHash, oldMR, oldGv, 2);
         (await nxms.getLatestAddress(toHex('CR')))
             .should.be.equal(newCr.address);
       });
    it('1.26 should able to propose new contract code for Pool1',
       async function() {
         ...
         let oldGv =
             await Governance.at(await nxms.getLatestAddress(toHex('GV')));
         actionHash =
             encode('upgradeContract(bytes2,address)', 'P1', newP1.address);
         await gvProp(29, actionHash, oldMR, oldGv, 2);
         (await nxms.getLatestAddress(toHex('P1')))
             .should.be.equal(newP1.address);
       });
    it('1.27 should able to propose new contract code for Pool2',
       async function() {
         ...
         let oldGv =
             await Governance.at(await nxms.getLatestAddress(toHex('GV')));
         actionHash =
             encode('upgradeContract(bytes2,address)', 'P2', newP2.address);
         await gvProp(29, actionHash, oldMR, oldGv, 2);
         (await nxms.getLatestAddress(toHex('P2')))
             .should.be.equal(newP2.address);
       });
    it('1.28 should able to propose new contract code for mcr',
       async function() {
         ...
         let oldGv =
             await Governance.at(await nxms.getLatestAddress(toHex('GV')));
         actionHash =
             encode('upgradeContract(bytes2,address)', 'MC', newMcr.address);
         await gvProp(29, actionHash, oldMR, oldGv, 2);
         (await nxms.getLatestAddress(toHex('MC')))
             .should.be.equal(newMcr.address);
       });
    it('1.29 should not trigger action if passed invalid address',
       async function() {
         let oldMR =
             await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
         let oldGv =
         ...
         actionHash =
             encode('upgradeContract(bytes2,address)', 'MC', ZERO_ADDRESS);
         await gvProp(29, actionHash, oldMR, oldGv, 2);
         (await nxms.getLatestAddress(toHex('MC'))).should.be.equal(mcrOld);
       });
    it('1.30 should not trigger action if passed invalid contrcat code',
       async function() {
         let oldMR =
             await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
         let oldGv =
         ...
         let mcrOld = await nxms.getLatestAddress(toHex('MC'));
         actionHash =
             encode('upgradeContract(bytes2,address)', 'P4', oldMR.address);
         await gvProp(29, actionHash, oldMR, oldGv, 2);
       });
  });

  describe('more test cases', function() {
    it('1.24 revert in case of upgrade implementation by non governance contract',
       async function() {
         await assertRevert(
    ...
    it('1.26 even if passed by governance should not trigger action for wrong contrcat code',
       async function() {
         this.timeout(0);
         let actionHash =
             encode('upgradeContractImplementation(bytes2,address)', 'AS',
                    nxms.address);
         let oldMR =
             await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
         let oldGv =
    ...
    it('1.27 revert in case of upgrade contract by non governance contract',
       async function() {
