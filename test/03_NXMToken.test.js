...
contract('NXMToken', function([
           owner, member1, member2, member3, notMember, spender2, govVoter1,
  ...
  describe('token details', function() {
    ...
    it('3.5 should return zero available balance for non member',
       async function() {
  ...
  describe('balanceOf', function() {
    describe('when the requested account is a member and has no tokens',
             function() {
    ...
    describe('when the requested account is a member and has some tokens',
             function() {
               beforeEach(async function() {});
               it('3.9 returns the non zero amount of tokens',
                  async function() {
                    (await tk.balanceOf(member1))
                        .toString()
                        .should.be.not.equal((0).toString());
      ...
      describe('when the sender is a member', function() {
        ...
        describe('when the sender does have enough balance', function() {
          ...
          it('3.14 emits a transfer event', async function() {
            ...
            event.args.value.toString().should.be.equal(
                transferTokens.toString());
          });
        });
      });
    });
  ...
  describe('approve', function() {
    describe('when the sender is a member', function() {
      ...
      describe('when approve amount is less than balance of sender',
               function() {
      ...
      describe('when approve amount is more than balance of sender',
               function() {
                 it('3.17 approves the requested amount', async function() {
                   const approveTokens =
                       new BN((await tk.balanceOf(member1)).toString())
      ...
      describe('when the sender is a member', function() {
        ...
        describe('when the recipient is a member', function() {
          ...
          describe('when the sender does not have enough balance', function() {
            it('3.25 reverts', async function() {
              ...
              await assertRevert(tk.transferFrom(
                  sender, to,
          ...
          describe('when the spender does not have enough approved balance',
                   function() {
                     ...
                     it('3.26 reverts', async function() {
                       await assertRevert(tk.transferFrom(sender, to, amount,
                                                          {from : spender}));
                     });
                   });
        });
        describe('when the recipient is ZERO_ADDRESS', function() {
          ...
          it('3.27 reverts', async function() {
            await assertRevert(tk.transferFrom(sender, ZERO_ADDRESS, amount,
                                               {from : spender}));
          });
        });
      });
  ...
  describe('Sell Tokens', function() {
    ...
    it('3.28 should able to sell tokens', async function() {
      ...
      const newPoolBalance = new BN(initialPoolBalance.toString())
                                 .sub(new BN(sellTokensWorth.toString()));
      const newTokenBalance = new BN(initialTokenBalance.toString())
                                  .sub(new BN(sellTokens.toString()));
      const newTotalSupply = new BN(initialTotalSupply.toString())
                                 .sub(new BN(sellTokens.toString()));
      newTokenBalance.toString().should.be.equal(
          (await tk.balanceOf(member1)).toString());
      newTotalSupply.toString().should.be.equal(
          (await tk.totalSupply()).toString());
    ...
    it('3.29 should not be to sell tokens more than balance', async function() {
      ...
      await assertRevert(P1.sellNXMTokens(
          new BN(tokenBalance.toString()).add(new BN(toWei(1).toString())),
          {from : member1}));
    });

    it('3.30 should not be to sell tokens more than maxSellTokens',
       async function() {
  ...
  describe('Burn', function() {
    describe('Tokens Less than balance', function() {
      it('3.31 should be able to burn tokens', async function() {
        ...
        const event = expectEvent.inLogs(logs, 'Transfer', {from : member1});
  ...
  describe('Misc', function() {
    describe('Buy Tokens at zero price', function() {
      before(async function() {
        ...
        await mcr.addMCRData(await getValue(toWei(2), pd, mcr), 0, toWei(2),
    ...
    describe('Emergency Pause', function() {
      before(
          async function() { await nxms.addEmergencyPause(true, '0x4142'); });

      after(
          async function() { await nxms.addEmergencyPause(false, '0x4142'); });
    });
    ...
    describe('Setter functions', function() {
      it('3.34 should be able to change joining fee', async function() {
        ...
        let oldGv =
            await Governance.at(await nxms.getLatestAddress(toHex('GV')));
        actionHash =
            encode('updateUintParameters(bytes8,uint)', toHex('JOINFEE'), 1);
      ...
      it('3.35 should be able to change BookTime', async function() {
        ...
        let oldGv =
            await Governance.at(await nxms.getLatestAddress(toHex('GV')));
        actionHash =
            encode('updateUintParameters(bytes8,uint)', toHex('CABOOKT'), 1);
        await gvProp(21, actionHash, oldMR, oldGv, 2);
      ...
      it('3.36 should be able to change lockCADays', async function() {
        ...
        let oldGv =
            await Governance.at(await nxms.getLatestAddress(toHex('GV')));
        actionHash =
            encode('updateUintParameters(bytes8,uint)', toHex('CALOCKT'), 1);
        await gvProp(21, actionHash, oldMR, oldGv, 2);
      ...
      it('3.37 should be able to change SCValidDays', async function() {
        ...
        let oldGv =
            await Governance.at(await nxms.getLatestAddress(toHex('GV')));
        actionHash =
            encode('updateUintParameters(bytes8,uint)', toHex('RALOCKT'), 1);
        await gvProp(21, actionHash, oldMR, oldGv, 2);
        let val = await td.getUintParameters(toHex('RALOCKT'));
        val[1].toString().should.be.equal((1).toString());
      });
      it('3.38 should be able to change LockTokenTimeAfterCoverExp',
         async function() {
           let oldMR =
               await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
           let oldGv =
               await Governance.at(await nxms.getLatestAddress(toHex('GV')));
           actionHash = encode('updateUintParameters(bytes8,uint)',
                               toHex('QUOLOCKT'), 1);
           await gvProp(20, actionHash, oldMR, oldGv, 2);
           let val = await td.getUintParameters(toHex('QUOLOCKT'));
           val[1].toString().should.be.equal((1).toString());
         });
      // it('3.39 should be able to change CanAddMemberAddress', async
      // function() {
      //   await assertRevert(
      ...
      it('3.40 only governance call should be able to change MVDays',
         async function() {
           let oldMR =
               await MemberRoles.at(await nxms.getLatestAddress(toHex('MR')));
           let oldGv =
               await Governance.at(await nxms.getLatestAddress(toHex('GV')));
           actionHash =
               encode('updateUintParameters(bytes8,uint)', toHex('MVLOCKT'), 1);
           await gvProp(21, actionHash, oldMR, oldGv, 2);
