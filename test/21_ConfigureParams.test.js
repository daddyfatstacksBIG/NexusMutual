...
const web3 = new Web3(new Web3.providers.HttpProvider(
    'http://localhost:8545')); // Hardcoded development port
...
contract('Configure Global Parameters', ([
  ...
  before(async function() {
    ...
    for (let i = 1; i < 4; i++) {
      ...
      await mr.payJoiningFee(
          web3.eth.accounts[i],
          {value : 2000000000000000, from : web3.eth.accounts[i]});
      await mr.kycVerdict(web3.eth.accounts[i], true, {from : ab1});
      await nxmToken.transfer(web3.eth.accounts[i], toWei(balances[i]));
    }
  });
  async function updateParameter(cId, mrSequence, code, contractInst, type,
                                 proposedValue) {
    ...
    if (type == 'address' || type == 'owner') {
      assert.equal(web3.toChecksumAddress(parameter[1]),
                   web3.toChecksumAddress(proposedValue));
    } else {
      assert.equal(parameter[1], proposedValue, 'Not updated');
    }
  }
  async function updateInvalidParameter(cId, mrSequence, code, contractInst,
                                        type, proposedValue) {
    code = web3.toHex(code);
    let getterFunction;
    if (type == 'uint') {
  ...
  describe('Update newly added Capital Model Parameters', function() {
    before(async function() {
      ...
      let actionHash = encode(
          ...
          'Description', 2, 50, 15, [ 2 ], 604800, '', mcr.address, toHex('MC'),
          [ 0, 0, 0, 1 ]);
      let p1 = await gv.getProposalLength();
      await gv.createProposalwithSolution('Add new member', 'Add new member',
                                          'Addnewmember', 3, 'Add new member',
                                          actionHash);
      ...
      await mcr.addMCRData(13001, '100000000000000000000',
    ...
    it('Should update Dynamic Mincap Threshold', async function() {
      ...
      await mcr.addMCRData(14003, '100000000000000000000',
                           '7000000000000000000000', [ '0x455448', '0x444149' ],
                           [ 100, 15517 ], 20190113);
      ((await mcr.variableMincap()) / 1e18)
          .toString()
      ...
      await mcr.addMCRData(15003, '100000000000000000000',
                           '7000000000000000000000', [ '0x455448', '0x444149' ],
                           [ 100, 15517 ], 20190113);
      ((await mcr.variableMincap()) / 1e18)
          .toString()
    ...
    it('Should not update newly added Capital Model Parameters',
       async function() {
  ...
  describe('Update Address Parameters', function() {
    it('Should update Master Contract Address', async function() {
      ...
      await updateParameter(27, 2, 'MASTADD', nxms, 'address',
                            newMaster.address, newMaster);
      (await gv.nxMasterAddress()).should.be.equal(newMaster.address);
      nxms = newMaster;
    });
    it('Should not trigger action if wrong code is passed', async function() {
      await updateInvalidParameter(27, 2, 'ASD', nxms, 'address',
                                   web3.eth.accounts[1]);
    });
    it('Should not trigger action if null address is passed', async function() {
      await updateInvalidParameter(27, 2, 'MASTADD', nxms, 'address',
                                   ZERO_ADDRESS);
    });
  });
  ...
  describe('Update Owner Parameters', function() {
    it('Should update Multi sig Wallet Address', async function() {
      await updateParameter(28, 3, 'MSWALLET', nxms, 'owner',
                            web3.eth.accounts[1]);
    });
    it('Should update MCR Notarise Address', async function() {
      await updateParameter(28, 3, 'MCRNOTA', nxms, 'owner',
                            web3.eth.accounts[1]);
    });
    ...
    it('Should update Uniswap Factory Address', async function() {
      ...
      await updateParameter(28, 3, 'UNISWADD', nxms, 'owner',
                            newUniswap.address);
    });
    it('Should update Owner Address', async function() {
      await updateParameter(28, 3, 'OWNER', nxms, 'owner',
                            web3.eth.accounts[1]);
    });
    it('Should update Quote Engine Address', async function() {
      await updateParameter(28, 3, 'QUOAUTH', nxms, 'owner',
                            web3.eth.accounts[1]);
    });
    it('Should update KYC Authorised Address', async function() {
      await updateParameter(28, 3, 'KYCAUTH', nxms, 'owner',
                            web3.eth.accounts[1]);
    });
    it('Should not trigger action if wrong code is passed', async function() {
      await updateInvalidParameter(28, 3, 'ASD', nxms, 'owner',
                                   web3.eth.accounts[1]);
    });
  });
});
