...
contract('MCR', function([ owner, notOwner ]) {
  ...
  describe('Token Price Calculation', function() {
    ...
    before(async function() {
      ...
      await mr.payJoiningFee(notOwner, {from : notOwner, value : toWei(0.002)});
      ...
      await mcr.addMCRData(9000, toWei(100), toWei(90),
    ...
    it('19.2 multiple tranches 100ETH', async function() {
      let x = await tk.balanceOf(notOwner);
      await p1.buyToken({from : notOwner, value : toWei(100)});
      let y = await tk.balanceOf(notOwner);
      console.log('multiple tranches 100ETH ==> ',
  ...
  describe('Token Price Calculation2', function() {
    ...
    before(async function() {
      ...
      await mcr.addMCRData(1000, toWei(100), toWei(10),
                           [ '0x455448', '0x444149' ], [ 100, 14800 ],
                           20190219);
    });
    it('19.3 single tranches 15 times Buy tokens', async function() {
      ...
      for (let i = 0; cost < toWei(180); i++) {
        ...
        console.log('tranche ', cost / toWei(1), ' ETH ==> ',
                    parseFloat(y - x) / toWei(1));
      }
    });
    it('19.4 tranches Buy more tokens', async function() {
      ...
      await mcr.addMCRData(202, toWei(30000), toWei(607.7406473491),
                           [ '0x455448', '0x444149' ], [ 100, 15517 ],
                           20190219);
      let x;
      let y;
      let cost = toWei(15);
      console.log('token rate 1ETH =  ',
                  toWei(1) /
                      parseFloat(await mcr.calculateTokenPrice(toHex('ETH'))));
      x = await tk.balanceOf(notOwner);
      await p1.buyToken({from : notOwner, value : cost});
      y = await tk.balanceOf(notOwner);
      console.log('tranche ', cost / toWei(1), ' ETH ==> ',
    ...
    it('19.5 Should revert while buying or 0  ETH',
       async function() { await assertRevert(p1.buyToken({value : 0})); });
  });

  describe('Token Selling', function() {
    it('19.6 Max sellable token will 0 if mcr percentage is less than 100',
       async function() {
    ...
    it('19.7 sell more than 1000 NXMs', async function() {
      ...
      await mcr.addMCRData(20000, toWei(100), poolBal[0],
                           [ '0x455448', '0x444149' ], [ 100, 15517 ],
                           20190219);
    ...
    it('19.6 Max sellable token will 0 if pool balance is less than 1.5 times of basemin',
       async function() {
         await tf.upgradeCapitalPool(DAI.address);
         parseFloat(await mcr.getMaxSellTokens()).should.be.equal(0);
       });
