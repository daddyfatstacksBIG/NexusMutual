...
contract('Claim: Assessment 2', function([
           owner,          underWriter1,   underWriter2,   underWriter3,
           underWriter4,   underWriter5,   underWriter6,   claimAssessor1,
           claimAssessor2, claimAssessor3, claimAssessor4, claimAssessor5,
           coverHolder1,   coverHolder2,   coverHolder3,   coverHolder4,
           coverHolder5,   coverHolder6,   coverHolder7,   coverHolder8,
           coverHolder9,   member1,        member2,        member3,
  ...
  before(async function() {
    ...
    await mcr.addMCRData(10000, 0, toWei(6000), [ ethereum_string, dai_string ],
                         [ 100, 2500 ], 20190208);
    ...
    // else await tc.burnFrom(owner, (await tk.totalSupply()) - 600000 *
    // toWei(1));
  ...
  describe('claim test case', function() {
    let UWarray = [
      underWriter1, underWriter2, underWriter3, underWriter4, underWriter5
    ];
    ...
    it('18.1 Should buy cover and collect rewards', async function() {
      ...
      async function updateUWDetails(changeInUWBalanceExpected) {
        for (let i = 0; i < UWarray.length; i++) {
          let currentUWBalance =
              parseFloat((await tk.balanceOf(UWarray[i])) / toWei(1));
      ...
      var vrsdata = await getQuoteValues(
          ...
          ethereum_string, 100, SC1, qt.address);
      await p1.makeCoverBegin(
          ...
          100, vrsdata[0], vrsdata[1], vrsdata[2],
      ...
      await p1.makeCoverUsingCA(
          ...
          100, vrsdata[0], vrsdata[1], vrsdata[2], {from : coverHolder3});
      ...
      vrsdata = await getQuoteValues(
          ...
          ethereum_string, 200, SC2, qt.address);
      await p1.makeCoverBegin(
          SC2, ethereum_string,
          [
            2, '26283367556000000', '200000000000000000000', '3549627424',
          ...
          200, vrsdata[0], vrsdata[1], vrsdata[2],
      ...
      await p1.makeCoverUsingCA(
          ...
          200, vrsdata[0], vrsdata[1], vrsdata[2], {from : coverHolder2});
      ...
      vrsdata = await getQuoteValues(
          ...
          ethereum_string, 300, SC3, qt.address);
      await p1.makeCoverBegin(
          SC3, ethereum_string,
          [
            3, '59137577002000000', '300000000000000000000', '3549627424',
          ...
          300, vrsdata[0], vrsdata[1], vrsdata[2],
      ...
      await p1.makeCoverUsingCA(
          ...
          300, vrsdata[0], vrsdata[1], vrsdata[2], {from : coverHolder6});
      ...
      vrsdata = await getQuoteValues(
          ...
          ethereum_string, 400, SC4, qt.address);
      await p1.makeCoverBegin(
          SC4, ethereum_string,
          [
            4, '105133470226000000', '400000000000000000000', '3549627424',
          ...
          400, vrsdata[0], vrsdata[1], vrsdata[2],
      ...
      await p1.makeCoverUsingCA(
          ...
          400, vrsdata[0], vrsdata[1], vrsdata[2], {from : coverHolder8});
      ...
      vrsdata = await getQuoteValues(
          ...
          ethereum_string, 500, SC5, qt.address);
      await p1.makeCoverBegin(
          SC5, ethereum_string,
          [
            5, '164271047228000000', '500000000000000000000', '3549627424',
          ...
          500, vrsdata[0], vrsdata[1], vrsdata[2],
    ...
    // it('18.2 Should not be able to updateStakerCommission if premiumNXM is
    // 0', async function() {
    //   await tf.updateStakerCommissions(SC1, 0, { from: owner });
    // });

    // it('18.3 Calling updateStakerCommissions when max commission is reached
    // which is in case of buying cover 7 for SC4', async function() {
    //   // after calling make cover begin for SC4, all UW's recived 50% (max)
    //   of their staked as commission so calling the funtion in the next line
    //   has no effect await tf.updateStakerCommissions(SC4,
    //   400000000000000000000, {
    //     from: owner
    //   });
    //   // the above function is simply run but has no effect for else part of
    //   if (maxCommission > commissionEarned)
    // });

    it('18.4 should pass for CA vote > 10 SA and majority > 70 % for reject(D1)',
       async function() {
         // (await nxms.isPause()).should.equal(false);

         class claimAssessor {
           constructor(initialDate, newLockDate, rewardRecieved,
         ...
         // try submitting the same claim again (to pass the TokenData.sol
         // setDepositCN's require condition of the coverage report) await
         // assertRevert(cl.submitClaim(coverID[0], { from: coverHolder5 }));
         await assertRevert(td.setDepositCN(coverID[0], true, {from : owner}));

         let now = await latestTime();
         claimID = (await cd.actualClaimLength()) - 1;

         claimAssessor1Object.initialDate =
             parseFloat(await tc.getLockedTokensValidity(claimAssessor1, CLA));
         claimAssessor2Object.initialDate =
             parseFloat(await tc.getLockedTokensValidity(claimAssessor2, CLA));
         claimAssessor3Object.initialDate =
             parseFloat(await tc.getLockedTokensValidity(claimAssessor3, CLA));
         ...
         closingTime =
             new BN(minVotingTime.toString()).add(new BN(now.toString()));
         await increaseTimeTo(
             new BN(closingTime.toString()).sub(new BN((10).toString())));

         await p1.__callback(APIID, '');

         assert.equal(parseFloat((await cd.getClaimStatusNumber(claimID))[1]),
                      0);

         // check the CA vote not closing before the minimum time is reached
         // even if the CA Vote is greater than 10*SA
         await increaseTimeTo(
             new BN(closingTime.toString()).add(new BN((10).toString())));

         let coverTokensLockedBefore =
             parseFloat(await tf.getUserLockedCNTokens(coverHolder5, coverID));
         let tokenBalanceBefore = parseFloat(await tk.balanceOf(coverHolder5));
         let totalBalanceBefore =
             parseFloat(await tc.totalBalanceOf(coverHolder5));
         let balanceBefore =
             parseFloat(await web3.eth.getBalance(coverHolder5));
         ...
         let totalBalanceAfter =
             parseFloat(await tc.totalBalanceOf(coverHolder5));
         let coverTokensLockedAfter =
             parseFloat(await tf.getUserLockedCNTokens(coverHolder5, coverID));
         ...
         claimAssessor1Object.rewardRecieved =
             parseFloat(
                 await cr.getRewardToBeDistributedByUser(claimAssessor1)) /
             toWei(1);
         claimAssessor2Object.rewardRecieved =
             parseFloat(
                 await cr.getRewardToBeDistributedByUser(claimAssessor2)) /
             toWei(1);
         claimAssessor3Object.rewardRecieved =
             parseFloat(
                 await cr.getRewardToBeDistributedByUser(claimAssessor3)) /
             toWei(1);
         ...
         for (let i = 0; i < UWarray.length; i++) {
           UWTokensLocked.push(
               (parseFloat(await tf.getStakerLockedTokensOnSmartContract(
                    UWarray[i], SC1, i)) -
                parseFloat(await tf.getStakerUnlockableTokensOnSmartContract(
                    UWarray[i], SC1, 0))) /
               toWei(1));
           UWTokensBurned[i] = UWTotalBalanceBefore[i] - UWTotalBalanceAfter[i];
         }

         assert.equal(claimAssessor1Object.newLockDate -
                          claimAssessor1Object.initialDate,
                      oneWeek);
         assert.equal(claimAssessor2Object.newLockDate -
                          claimAssessor2Object.initialDate,
                      oneWeek);
         assert.equal(claimAssessor3Object.newLockDate -
                          claimAssessor3Object.initialDate,
                      oneWeek);
         ...
         // if ((await tk.totalSupply()) < 600000 * toWei(1))
         //   await tc.mint(owner, 600000 * toWei(1) - (await
         //   tk.totalSupply()));
         // else await tc.burnFrom(owner, (await tk.totalSupply()) - 600000 *
         // toWei(1));
       });

    it('18.5 should pass for CA vote > 10 SA and majority > 70 % for accept(A1)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
         ...
         // await tc.lock(CLA, 50000 * toWei(1), validity, {from:
         // claimAssessor1}); await tc.lock(CLA, 30000 * toWei(1),
         // validity, {from: claimAssessor2}); await tc.lock(CLA, 20000 *
         // toWei(1), validity, {from: claimAssessor3});
         ...
         let now = await latestTime();
         closingTime =
             new BN(maxVotingTime.toString()).add(new BN(now.toString()));
         await increaseTimeTo(
         ...
         coverTokensBurned = Number(
             ((totalBalanceBefore - totalBalanceAfter) / toWei(1)).toFixed(2));
         payoutReceived =
             Number(((balanceAfter - balanceBefore) / toWei(1)).toFixed(2));
         ...
         for (let i = 0; i < UWarray.length; i++) {
           UWTokensBurned[i] =
               Number(UWTotalBalanceBefore[i] - UWTotalBalanceAfter[i])
                   .toFixed(2);
         }
         ...
         assert.equal(parseFloat(await td.getStakerStakedBurnedByIndex(
                          underWriter1, 0)) /
                          toWei(1),
                      2000);

         // befor the last burn happened, all UW 2000 were staked and none was
         // unlocked befor the voting closed.
         assert.equal(
             parseFloat(await td.getStakerStakedUnlockableBeforeLastBurnByIndex(
                 underWriter1, 0)) /
                 toWei(1),
             0);
    ...
    it('18.6 should pass for CA vote > 10 SA and majority < 70%, open for member vote and majority reject(D3)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
         ...
         class member {
           constructor(rewardRecieved) { this.rewardRecieved = rewardRecieved; }
         }
         let claimAssessor1Object = new claimAssessor();
         let claimAssessor2Object = new claimAssessor();
         let claimAssessor3Object = new claimAssessor();
         ...
         for (let i = 0; i < UWarray.length; i++) {
           UWTokensLocked.push(
               Number((parseFloat(await tf.getStakerLockedTokensOnSmartContract(
                           UWarray[i], SC1, i)) -
                       parseFloat(
         ...
         member1Object.rewardRecieved =
             parseFloat(await cr.getRewardToBeDistributedByUser(member1)) /
             toWei(1);
         member2Object.rewardRecieved =
             parseFloat(await cr.getRewardToBeDistributedByUser(member2)) /
             toWei(1);
         member3Object.rewardRecieved =
             parseFloat(await cr.getRewardToBeDistributedByUser(member3)) /
             toWei(1);
         ...
         //   tk.totalSupply()));
         // else await tc.burnFrom(owner, (await tk.totalSupply()) - 600000 *
         // toWei(1)); now = await latestTime(); await increaseTimeTo(now+(await
         // td.bookTime())/1+10);
       });

    it('18.7 should pass for CA vote > 10 SA and majority < 70%, open for member vote and majority accept(A3)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
    ...
    it('18.8 should pass for CA vote > 10 SA and majority < 70%, open for member vote and MV<5 SA and CA majority reject(D4)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
         ...
         UWarray = [
           underWriter3, underWriter2, underWriter5, underWriter4, underWriter1
         ];
         // need not to do the lock again
         // await tc.lock(CLA, 50000 * toWei(1), validity, {from:
         // claimAssessor1}); await tc.lock(CLA, 30000 * toWei(1),
         ...
         claimAssessor3Object.initialDate =
             parseFloat(await tc.getLockedTokensValidity(claimAssessor3, CLA));
         claimAssessor4Object.initialDate =
             parseFloat(await tc.getLockedTokensValidity(claimAssessor4, CLA));
         claimAssessor5Object.initialDate =
             parseFloat(await tc.getLockedTokensValidity(claimAssessor5, CLA));
         await cl.submitCAVote(claimID, 1, {from : claimAssessor1});
         await cl.submitCAVote(claimID, 1, {from : claimAssessor2});
         await cl.submitCAVote(claimID, 1, {from : claimAssessor3});
         ...
         let coverTokensLockedBefore =
             parseFloat(await tf.getUserLockedCNTokens(coverHolder1, coverID));
         let tokenBalanceBefore = parseFloat(await tk.balanceOf(coverHolder1));
         let balanceBefore =
             parseFloat(await web3.eth.getBalance(coverHolder1));
         let totalBalanceBefore =
             parseFloat(await tc.totalBalanceOf(coverHolder1));
         ...
         claimAssessor4Object.rewardRecieved =
             parseFloat(
                 await cr.getRewardToBeDistributedByUser(claimAssessor4)) /
             toWei(1);
         claimAssessor5Object.rewardRecieved =
             parseFloat(
                 await cr.getRewardToBeDistributedByUser(claimAssessor5)) /
             toWei(1);
         ...
         coverTokensBurned =
             Number((totalBalanceBefore - totalBalanceAfter) / toWei(1))
                 .toFixed(2);
         payoutReceived =
             Number((balanceAfter - balanceBefore) / toWei(1)).toFixed(2);
         coverTokensUnlockable =
             Number((tokenBalanceAfter - tokenBalanceBefore) / toWei(1))
                 .toFixed(2);
         ...
         assert.equal(claimAssessor3Object.newLockDate -
                          claimAssessor3Object.initialDate,
                      oneWeek);
         assert.equal(claimAssessor4Object.newLockDate -
                          claimAssessor4Object.initialDate,
                      oneWeek);
         assert.equal(claimAssessor5Object.newLockDate -
                          claimAssessor5Object.initialDate,
                      oneWeek);
    ...
    it('18.9 should pass for CA vote > 10 SA and majority < 70%, open for member vote and MV<5 SA and CA majority accept(A4)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
         ...
         // await tc.lock(CLA, 60000 * toWei(1), validity, {from:
         // claimAssessor4}); await tc.lock(CLA, 50000 * toWei(1),
         // validity, {from: claimAssessor5});
         for (let i = 0; i < UWarray.length; i++) {
           UWTotalBalanceBefore[i] =
               parseFloat(await tc.totalBalanceOf(UWarray[i])) / toWei(1);
         }
    ...
    it('18.10 should pass for CA vote > 5SA and <10 SA and majority < 70%, open for member vote and majority reject(D3)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
         ...
         let coverTokensLockedBefore =
             parseFloat(await tf.getUserLockedCNTokens(coverHolder2, coverID));
         ...
         let totalBalanceBefore =
             parseFloat(await tc.totalBalanceOf(coverHolder2));
         ...
         member4Object.rewardRecieved =
             parseFloat(await cr.getRewardToBeDistributedByUser(member4)) /
             toWei(1);
         member5Object.rewardRecieved =
             parseFloat(await cr.getRewardToBeDistributedByUser(member5)) /
             toWei(1);
    ...
    it('18.11 should pass for CA vote > 5SA and <10 SA and majority < 70%, open for member vote and majority accept(A3)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
    ...
    it('18.12 should pass for CA vote > 5* SA and <10 SA and majority > 70 % for reject(D1)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
         ...
         UWarray = [
           underWriter5, underWriter4, underWriter3, underWriter2, underWriter1
         ];
         // need not to do the lock again
         // await tc.lock(CLA, 50000 * toWei(1), validity, {from:
         // claimAssessor1}); await tc.lock(CLA, 30000 * toWei(1),
         ...
         let coverTokensLockedBefore =
             parseFloat(await tf.getUserLockedCNTokens(coverHolder4, coverID));
         ...
         let totalBalanceBefore =
             parseFloat(await tc.totalBalanceOf(coverHolder4));
    ...
    it('18.13 should pass for CA vote > 5* SA and <10 SA and majority > 70 % for accept(A1)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
    ...
    it('18.14 should pass for CA vote < 5* SA and MV < 5 SA and CA majority reject(D4)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
         ...
         let coverTokensLockedBefore =
             parseFloat(await tf.getUserLockedCNTokens(coverHolder6, coverID));
         ...
         let totalBalanceBefore =
             parseFloat(await tc.totalBalanceOf(coverHolder6));
    ...
    it('18.15 should pass for CA vote < 5* SA and MV < 5 SA and CA majority accept(A4)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
    ...
    it('18.16 should pass for 0 CA votes, MV < 5 SA(D4)', async function() {
      ...
      UWarray = [
        underWriter4, underWriter3, underWriter5, underWriter2, underWriter1
      ];
      ...
      let coverTokensLockedBefore =
          parseFloat(await tf.getUserLockedCNTokens(coverHolder7, coverID));
      ...
      let totalBalanceBefore =
          parseFloat(await tc.totalBalanceOf(coverHolder7));
      ...
      for (let i = 0; i < UWarray.length; i++) {
        UWTokensBurned[i] =
            Number(UWTotalBalanceBefore[i] - UWTotalBalanceAfter[i]).toFixed(2);
      }
    ...
    it('18.17 should pass for CA vote > 5 SA and CA<10 SA and majority < 70%, open for member vote and MV<5 SA and CA majority reject(D4)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
         ...
         let tokenBalanceBefore = parseFloat(await tk.balanceOf(coverHolder7));
         let balanceBefore =
             parseFloat(await web3.eth.getBalance(coverHolder7));
         let totalBalanceBefore =
             parseFloat(await tc.totalBalanceOf(coverHolder7));
    ...
    it('18.18 should pass for CA vote > 5 SA and CA<10SA majority < 70%, open for member vote and MV<5 SA and CA majority accept(A4)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
         ...
         let coverTokensLockedBefore =
             parseFloat(await tf.getUserLockedCNTokens(coverHolder8, coverID));
         ...
         let totalBalanceBefore =
             parseFloat(await tc.totalBalanceOf(coverHolder8));
    ...
    it('18.19 CA vote<5SA, open for member vote and majority reject(D3)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
         ...
         let coverTokensLockedBefore =
             parseFloat(await tf.getUserLockedCNTokens(coverHolder9, coverID));
         let tokenBalanceBefore = parseFloat(await tk.balanceOf(coverHolder9));
         let balanceBefore =
             parseFloat(await web3.eth.getBalance(coverHolder9));
         let totalBalanceBefore =
             parseFloat(await tc.totalBalanceOf(coverHolder9));
         ...
         member6Object.rewardRecieved =
             parseFloat(await cr.getRewardToBeDistributedByUser(member6)) /
             toWei(1);
    ...
    it('18.20 CA vote <5SA and majority < 70%, open for member vote and majority accept(A3)',
       async function() {
         // (await nxms.isPause()).should.equal(false);
  ...
  describe('Burning 0 tokens of a staker', function() {
    ...
    it('18.25 when stakerStakedNXM = 0', async function() {
      ...
      let now = await latestTime();
      closingTime = new BN(maxVotingTime.toString())
                        .add(new BN((now + maxStakeTime).toString()));
      await increaseTimeTo(closingTime);
      await tf.burnStakerLockedToken(SC1, 10);
    });
    it('18.26 when stakerStakedNXM = 0',
       async function() { await assertRevert(p1.depositCN(0)); });
  });
});
